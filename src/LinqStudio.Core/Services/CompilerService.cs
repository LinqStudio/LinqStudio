...
public class CompilerService : IDisposable
{
	...
	public async Task<IReadOnlyList<(CompletionItem Item, string? Description)>> GetCompletionsAsync(string userQueryContent, int cursorPosition)
	{
		...
		try
		{
			...
			var block = new TransformBlock<CompletionItem, (CompletionItem, string?)>(async completion =>
			{
				var description = await completionService.GetDescriptionAsync(document, completion);
				return (completion, description?.Text);
			}, new ExecutionDataflowBlockOptions
			{
				EnsureOrdered = true,
				MaxDegreeOfParallelism = 10 // Limit concurrent description fetches to avoid overwhelming Roslyn
			});

			foreach (var completion in completionList.ItemsList)
			{
				await block.SendAsync(completion);
			}

			block.Complete();
			return await block.ReceiveAllAsync().ToListAsync();
		}
		finally
		{
			_lock.Release();
		...
		try
		{
			...
			switch (symbol)
			{
				case Microsoft.CodeAnalysis.IMethodSymbol m:
					display = m.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat);
					break;
				case Microsoft.CodeAnalysis.IPropertySymbol p:
					display = $"{p.Type.ToDisplayString(SymbolDisplayFormat.CSharpErrorMessageFormat)} {p.Name}";
					break;
				default:
