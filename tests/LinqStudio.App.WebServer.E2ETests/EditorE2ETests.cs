...
public class EditorE2ETests(AppServerFixture app, PlaywrightFixture pw)
{
	...
	public async Task Editor_Hover_ShowsSymbolInfo()
	{
		...
		await E2ETestHelpers.ClearAndWriteQueryAsync(page, $"{E2ETestHelpers.SqliteContextParameterName}.People.Where(");
		...
		var whereToken = page.Locator(E2ESelectors.MonacoToken).Filter(new() { HasText = "Where", HasNotText = E2ETestHelpers.SqliteContextParameterName });
	...
	public async Task Editor_AutoTriggers_CompletionOnDot()
	{
		...
		// Clear the editor first and type some code
		await E2ETestHelpers.ClearAndWriteQueryAsync(page, $"{E2ETestHelpers.SqliteContextParameterName}.People");
	...
	public async Task Editor_AutoTriggers_CompletionOnSpace()
	{
		...
		await E2ETestHelpers.ClearAndWriteQueryAsync(page, $"{E2ETestHelpers.SqliteContextParameterName}.People.Where( x => x => x.Age");
		await page.Keyboard.TypeAsync(" ");
	...
	public async Task Editor_ShowsUnsavedIndicator_WhenQueryModified()
	{
		...
		await E2ETestHelpers.ClearAndWriteQueryAsync(page, $"{E2ETestHelpers.SqliteContextParameterName}.People.Where(x => x.Id > 0)");
